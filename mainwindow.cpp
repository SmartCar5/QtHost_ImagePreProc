...
void MainWindow::reNew()
{
    ...
    try
    {
        if(!image.empty()&&isChanged==true&&changeCont>=10)
        {
            undist();
        }
    }
...
void MainWindow::on_pushButtonSave_clicked()
{
    ...
    try
    {
        ...
        arguments<<"notepad "+path+"\n";//
        cout<<arguments.at(0).toStdString();
        QProcess process1(this);
        process1.startDetached("cmd.exe",arguments);*/
...
void MainWindow::on_pushButtonUndistort_clicked()
{
    ...
    QMessageBox msg;
    msg.about(NULL,"INFO",QString("%1 pictures loaded").arg(list.size()));
    QInputDialog log;
    ...
    for (unsigned long long i = 0; i < imgs.size(); i++)
    {
        ...
        ui->progressBar->setValue(5+(float)i/imgs.size()*55);
    }
    msg.about(NULL,"INFO",QString("%1 pictures available").arg(imgs.size()));
    //生成棋盘格每个内角点的空间三维坐标
    ...
    for (unsigned long long i = 0; i < imgsPoints.size(); i++)
    {
        ...
        ui->progressBar->setValue(60+(float)i/imgsPoints.size()*10);
    }
...
void MainWindow::wheelEvent(QWheelEvent *event)
{
    if(event->delta() > 0)
    {
        ...
        {
            angle-=0.01;
            (angle<=0.01)?(angle = 0.01):angle;
            ui->lineEdit->setText(QString("%1").arg(angle));
        }
        else
        {
            j_large-=0.01;
            (j_large<0)?(j_large = 0):j_large;
            ui->lineEdit_3->setText(QString("%1").arg(j_large));
        }
    }
    else
    {
...
{
    if((event->buttons()&Qt::LeftButton)&&(event->buttons()&Qt::LeftButton))
    {
        mouseFlag = true;
    }
    else
...
void MainWindow::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(mouseFlag==true)
    {
        ...
        (dep<0.01)?(dep = 0.01):dep;
        ui->lineEdit_2->setText(QString("%1").arg(dep));
        if(!image.empty())
        {
            transp();
        }
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    if(event->key()==Qt::Key_Up)
    {
        if(i_abodon<110)
        i_abodon+=1;
        ui->lineEdit_4->setText(QString("%1").arg(i_abodon));
        if(!image.empty())
        {
            transp();
        }
